...
const Login = ({ mode, toggleMode }) => {
  ...
  return (
    <>
      {value ? <Home mode={mode} toggleMode={toggleMode} value={value} /> :
            ...
            <div>
              <form onSubmit={handleSubmit}>
                <div className="text-start mt-4">
                  <Link onClick={handleSubmit} className={`btn btn-outline-${mode==='dark'?'warning':'primary my-2'}`} style={{fontSize: "20px", fontWeight: 'initial'}}>Login with <FcGoogle /></Link>
